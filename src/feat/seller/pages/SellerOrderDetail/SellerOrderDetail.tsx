  ...
  const handleStatusUpdate = async (newStatus: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Order status changed to ${newStatus}. Notification sent via Email & SMS.`,
        variant: "default"
      });
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border shadow-sm overflow-hidden bg-card">
            ...
            <CardContent className="p-0">
              ...
              {/* ─── Pricing Breakdown ─────────────────────────────────── */}
              <div className="space-y-2 p-4 bg-muted/20 border-t">
                <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 mb-3">Pricing Breakdown</p>

                {/* Listing Price */}
                <div className="flex flex-row items-center justify-between text-sm text-muted-foreground w-full">
                  <span>Listing Price (MRP)</span>
                  <span className="font-medium text-foreground">₹{listingTotal.toLocaleString()}</span>
                </div>
                {/* Selling Price */}
                <div className="flex flex-row items-center justify-between text-sm text-muted-foreground w-full">
                ...
                <div className="flex flex-row items-center justify-between text-sm text-muted-foreground w-full">
                  <span className="pl-3 text-[12px]">Platform Fee (2%)</span>
                  <span className="font-medium text-foreground">₹{platformFee.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
                </div>
                ...
                <div className="flex flex-row items-center justify-between w-full">
                  ...
                  <span className="font-bold text-lg text-[#0E6C68]">₹{total.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
                </div>

                {/* Seller Payout */}
                <div className="flex flex-row items-center justify-between w-full bg-emerald-50 dark:bg-emerald-900/20 rounded-lg px-3 py-2 border border-emerald-200 dark:border-emerald-800">
                  <span className="text-sm font-bold text-emerald-800 dark:text-emerald-300">Seller Payout (est.)</span>
                  <span className="font-black text-base text-emerald-700 dark:text-emerald-400">₹{sellerPayout.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</span>
                </div>
