                ...
                <TabsContent value="notifications" className="mt-0 space-y-6">
                  <Card>
                    ...
                    <CardContent className="space-y-6">
                      ...
                      {/* ─── SMS Section ────────────────────────────── */}
                      <div className="pt-4">
                        <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40 mb-3 pl-1">SMS & Mobile</p>
                        <div className="space-y-3">
                          <div className="flex items-center justify-between p-4 border rounded-2xl hover:bg-muted/30 transition-all">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                <p className="font-bold text-base leading-tight">Mobile Notifications (SMS)</p>
                                <p className="text-xs text-muted-foreground mt-1">Get critical order alerts and security updates via SMS to your mobile number.</p>
                              </div>
                            </div>
                            <Switch
                              checked={preferences?.smsNotifications}
                              onCheckedChange={(checked) => handlePreferencesSave({ smsNotifications: checked })}
                              disabled={prefsLoading || prefsUpdating}
                            />
                          </div>
                          <div className="flex items-center justify-between p-4 border rounded-2xl hover:bg-muted/30 transition-all bg-gradient-to-r from-orange-50/40 to-transparent dark:from-orange-900/10">
                            <div className="flex items-center gap-4">
                              <div className="bg-orange-500/10 p-2.5 rounded-xl">
                                <Megaphone className="w-5 h-5 text-orange-500" />
                              </div>
                              <div>
                                <p className="font-bold text-base leading-tight">Sales Booster Messages</p>
                                <p className="text-xs text-muted-foreground mt-1">Receive promotional SMS for ads, flash sales, and marketing campaigns to boost your store.</p>
                              </div>
                            </div>
                            <Switch
                              checked={preferences?.salesBoosterSms}
                              onCheckedChange={(checked) => handlePreferencesSave({ salesBoosterSms: checked })}
                              disabled={prefsLoading || prefsUpdating}
                            />
                          </div>
                        </div>
