...
export function CheckoutPage() {
    ...
    const [couponType, setCouponType] = useState<"percent" | "flat">("flat");
    ...
    // FIX #10: coupon discount calculation
    const couponDiscountAmount = couponApplied
        ? couponType === "percent"
            ? Math.round(actualSubtotal * couponDiscount / 100)
            : couponDiscount
        : 0;
    ...
    const applyCoupon = async () => {
        ...
        setCouponError("");
        try {
            const res = await apiClient.post("/coupons/validate", { code: couponCode.trim(), cartTotal: actualSubtotal });
            if (res.data.success) {
                setCouponDiscount(res.data.discount);
                setCouponType(res.data.type || "flat");
    ...
    const handleOnlinePayment = async (amount: number, items: any[]) => {
        ...
        try {
            ...
            const { razorpayOrderId, amount: rzpAmount, currency, orderId, orderNumber: nestedOrderNumber } = orderResponse.data.data || orderResponse.data;
                        ...
                        <CardContent className="space-y-4">
                            ...
                            <div className="space-y-2">
                                ...
                                {!couponApplied ? (
                                    <div className="space-y-1.5">
                                        <div className="flex gap-2">
                                            ...
                                            <Button size="sm" variant="outline" className="h-8 px-3 text-xs shrink-0"
                                                onClick={applyCoupon} disabled={applyingCoupon}>
                                    ...
                                    <div className="flex justify-between items-center text-sm text-green-600 bg-green-50 dark:bg-green-950/30 rounded-md px-2 py-1.5">
                                        <span className="font-medium">🎟️ {couponCode} applied!</span>
                                        <div className="flex items-center gap-2">
                                            <span className="font-bold">-₹{couponDiscountAmount}</span>
                                            <button onClick={removeCoupon} className="text-muted-foreground hover:text-destructive text-xs underline">Remove</button>
                                        </div>
                                    </div>
                                )}
