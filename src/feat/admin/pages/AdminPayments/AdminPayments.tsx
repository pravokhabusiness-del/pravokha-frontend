...
export default function AdminPayments() {
  ...
  const handleUpdatePayoutStatus = async (payoutId: string, newStatus: string) => {
    const request = payoutRequests.find(r => r.id === payoutId);

    // Guard: block approval if amount is below minimum threshold
    if ((newStatus === 'processing' || newStatus === 'completed') && request && request.amount < MIN_PAYOUT_AMOUNT) {
      toast({
        title: "⚠️ Minimum Amount Not Met",
        description: `Payout amount ₹${request.amount.toLocaleString()} is below the minimum threshold of ₹${MIN_PAYOUT_AMOUNT.toLocaleString()}. Cannot approve.`,
        variant: "destructive"
      });
      return;
    }
