...
import { Plus, Edit, Trash2, Upload, ImageIcon, Loader2, ArrowLeft, ExternalLink, Package } from "lucide-react";
...
export default function AdminComboOffers() {
  ...
  const fetchOffers = async () => {
    try {
      ...
      const { data } = await apiClient.get('/combo-offers?all=true');
          ...
          >
            {offers.map((offer) => (
              <Card key={offer.id} className="group flex flex-col overflow-hidden bg-card border-border/60 hover:shadow-lg transition-all duration-300 rounded-xl">
                {/* Combo Hero Banner / Product Thumbnails */}
                <div className="relative h-48 bg-muted/30 overflow-hidden border-b border-border/40">
                  {offer.products && offer.products.length > 0 ? (
                    // Show product thumbnail strips when we have enriched product data
                    <div className="flex h-full w-full">
                      {offer.products.slice(0, 3).map((product, idx) => (
                        <a
                          ...
                          className="relative flex-1 overflow-hidden group/thumb border-r border-white/10 last:border-r-0"
                        ...
                        >
                          {product.imageUrl ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center bg-muted/40 gap-1">
                              <Package className="h-6 w-6 text-muted-foreground/40" />
                              <span className="text-[9px] text-muted-foreground/60 font-medium px-1 text-center line-clamp-2">{product.title}</span>
                            </div>
                          )}
                          {/* Hover overlay with product name */}
                          <div className="absolute inset-0 bg-black/0 group-hover/thumb:bg-black/50 transition-all duration-200 flex items-end">
                            <div className="w-full p-1.5 translate-y-full group-hover/thumb:translate-y-0 transition-transform duration-200">
                              <p className="text-[9px] font-bold text-white line-clamp-1 leading-tight">{product.title}</p>
                              <div className="flex items-center gap-0.5 mt-0.5">
                                <ExternalLink className="h-2.5 w-2.5 text-white/70" />
                                <span className="text-[8px] text-white/70">View product</span>
                              </div>
                            </div>
                          </div>
                          {/* Product index badge */}
                ...
                <CardContent className="p-4 sm:p-5 pt-0 flex flex-col flex-1 gap-4">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2 bg-muted/20 p-3 rounded-xl border border-border/50">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-medium">{offer.productIds.length} items</p>
                    </div>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {offer.products.map((p) => (
                        <a
                          ...
                          rel="noopener noreferrer"
                          onClick={(e) => e.stopPropagation()}
                          className="inline-flex items-center gap-0.5 text-[10px] font-medium bg-primary/5 hover:bg-primary/15 text-primary border border-primary/15 px-1.5 py-0.5 rounded-md transition-colors"
                        >
                          {p.title}
