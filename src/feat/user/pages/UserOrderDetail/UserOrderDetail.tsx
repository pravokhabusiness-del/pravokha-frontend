...
export default function UserOrderDetail() {
   ...
   const handleConfirmCancellation = async (reason: string, comments: string) => {
      try {
         ...
         toast({
            ...
            description: "Your order has been cancelled successfully. A confirmation message has been sent to your registered Email and Phone via SMS.",
         });
   ...
   const handleReturnOrder = async () => {
      if (!confirm("Are you sure you want to return this order? Our support team will review your request within 24 hours.")) return;

      try {
         await apiClient.post(`/orders/${orderId}/return`, { reason: "Customer requested return via dashboard" });

         toast({
            title: "Return Request Submitted",
            description: "Your return request has been initiated. Our team will review it and contact you shortly with return instructions. We are sorry to see this item go back, but we want you to be completely satisfied.",
         });
         loadOrder();
      } catch (error: any) {
         ...
         toast({
            ...
            description: error.response?.data?.message || "Could not initiate return. Please contact support.",
            variant: "destructive",
         });
      }
   };
         ...
         <div className="mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            ...
            <div className="flex flex-col gap-3 w-full sm:w-auto sm:flex-row mt-4 sm:mt-0">
               ...
               {order.status?.toLowerCase() === 'delivered' && (
                  <Button variant="outline" size="sm" onClick={handleReturnOrder} className="h-10 w-full sm:w-auto justify-center sm:min-w-[120px] text-orange-600 border-orange-200 hover:bg-orange-50 dark:hover:bg-orange-950/30">
