...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('clear_cart', handleClearCart);
    return () => window.removeEventListener('clear_cart', handleClearCart);
  }, []);
  ...
  comboOffers.forEach(offer => {
    ...
    if (productIds.length > 0) {
      ...
      if (maxComboSets > 0 && maxComboSets !== Infinity) {
        ...
        const singleSetOriginal = productIds.reduce((sum, id) => {
          const product = comboProducts.find(p => p.productId === id);
          return sum + (product ? product.price : 0);
        }, 0);

        const singleSetDiscount = singleSetOriginal - offer.comboPrice;

        if (singleSetDiscount > 0) {
          comboSavings += singleSetDiscount * maxComboSets;
          // Consume items
